...
class MorningAlgorythm:
    ...
    @classmethod
    async def morning(cls, week: str, current_schedule: list[dict[str: Arrow, str: str]]) -> bool:
        ...
        for i, time in enumerate(times):
            times[i] = time['time'].shift(minutes=-subject_advance_time-5)
        ...
        for send_time in times:
            ...
            if delta.days < 0:
                if (send_time.shift(minutes=subject_advance_time) - get_now()).days < 0:
                    return False
                else:
                    delta = timedelta()
            ...
            if not await morning(week, get_now().weekday()):
                return False

        return True


async def main(week: str, current_schedule: list[dict[str: Arrow, str: str]]):
